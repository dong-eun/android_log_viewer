...
- 현재까지 수신한 전체 로그를 `YYYYMMDDhhmm_기기명.txt`로 저장
- 선택한 기기의 `dumpsys` TXT 및 `bugreport` ZIP 저장
- 로그 최대 50,000줄 유지로 장시간 실행 시 메모리 사용 제한
...
VS Code에서 `njpwerner.autodocstring` 확장을 사용합니다. 프로젝트를 열면 권장 확장으로 표시되며, 설정은 Google docstring 형식으로 자동 적용됩니다.
...
5. **현재 로그 저장**은 필터와 화면 지우기 여부에 관계없이 현재까지 수신한 전체 로그를 UTF-8 TXT로 저장합니다.
